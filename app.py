...
with st.sidebar:
    ...
    for i, response in enumerate(st.session_state.bot_responses):
...
if user_question and not st.session_state.cleared:
    ...
    st.session_state.bot_responses.append(answer)

st.markdown("### 💬 Chat History")
for speaker, message in st.session_state.chat_history:
    st.markdown(f"**{speaker}:** {message}")

# --- Display selected response from sidebar ---
if st.session_state.selected_response:
    st.markdown("---")
    st.markdown("### 📥 Selected Previous Bot Response")
    st.info(st.session_state.selected_response)
